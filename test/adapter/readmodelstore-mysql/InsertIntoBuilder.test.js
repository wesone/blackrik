...
test('create insert into statement with position check', () => {
    ...
    const expectedSQL = 'INSERT INTO `TestTable` (`test`, `testDate`, `testInt`, `testNull`, `testUndefined`, `_lastPosition`) SELECT ?, ?, ?, ?, ?, ? WHERE ? > COALESCE(( SELECT MAX( `_lastPosition` ) FROM `TestTable` ),0)';
    const expectedParameters = [
        'Hello world',
        '2021-12-17 02:24:00',
        '42',
